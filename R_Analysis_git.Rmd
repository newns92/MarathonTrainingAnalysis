...
```{r avg pace by runs - seperate, echo = FALSE}
ggplot(data = runs, aes(x = Date, y = Avg.Pace, colour = RunType)) + 
  geom_point() +
  geom_smooth(fill = NA) +  
  theme(axis.text.x = element_blank()) +
...
So we see the majority of runs I had were "normal" runs, and then recovery runs. But then in the bar chart, we can see that even though I only had *one* race, it had more steps than *all* of my recovery runs. Also, even with me having three times as many normal runs than long runs, the steps in long runs was over half of the steps from the normal runs. But the fact that the I had more steps in the marathon than in all of my recovery runs was pretty surprising. 

```{r cadence by hr, echo = FALSE}
...
```{r elevation and avg hr, echo = FALSE}}
