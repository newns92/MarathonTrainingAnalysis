...
Just like [last year](https://stevenewns.netlify.com/2017/04/18/marathon-training-plan-data-analysis/), I again can see the actual marathon itself and my one 24 mile run, my longest **Long Run** of the plan, all the way to the right of the plot. So, this distribution is right/positively-skewed, with the majority of the runs being less than 12 miles. So, as expected due to the higher mileage plan, the histogram has some higher bars, indicating a larger number of runs overall, with bars for the same distance being noticebaly higher for 2017 compared to 2016. 

The majority of my runs this training block were around 4-6 miles, as noted by the histogram for 2017, with the next highest frequency being from 10-12 miles. The fact that most of my runs were from 4-6 miles is obvvoisu when noting that the majority of my runs were recovery runs, which went from a minimum of 3 miles up to a maxximum of 7 miles (for them ,ost part, my mistkae)
...
The above shows that I had less workouts, long runs, middle long runs (longer than a general aerobic run, shorter than a long run), but more recovery runs and general aerobic runs. This should indicate that I should've been taking it a bit easier in this build-up, although some of the lower numbers for the workouts were due to missing these runs thanks to injury. For example, let's look at those runs that fall under `Misc`:
```{r Misc runs, echo = F}
philly %>%
  filter(RunCat == "Misc")
```

4 of these runs were the warm up and cool downs to tune-up races, but the other 2 were runs I kept short due to injury, one physical, one mental. I started to realize you don't have to hit *every* run on the schedule during the span of this plan, as noted by the empty space in the middle of September and October where I took some days off due to the recurring quad injury noted above on that run I cut short on September 13.
...
So, since I have a skewed distribution, I'd want to look at median as my measure of center. I can see the median number of miles ran in a run is 7 miles, which is lower than last year, which I can infer was due to more recovery runs this time around during the double-run days. And agian, see the max distance here is, of course, the actual marathon (but something must've been up with my GPS watch or the course, as it says I ran about .15 miles moreso than the actual race distance). 
...
Somewhat supringsly, my total mies in Novelmber was acutally shorter this time aroudn, while the other months were all larger. I expected septermebt and ocotber to be less than august this go aroudn due to the taking off of a couple days due to injiry, so that wasn't too much of a surpise


```{r peak week, echo = FALSE, fig.align='center'}
ggplot(data = philly, aes(Week, Distance, fill = Month)) + 
  geom_bar(stat="identity") +
  xlab("Week of Plan") + 
  ylab("Total Miles") + 
...
Long drops in Nov = feeling beat up (easy days too hard)
