...
# SOME POINTLESS CHARasdfTS REMOVED
fda
First, I loaded in the CSV file created in the **R_Cleaning_Garmin_Strava** script file, which contains the combined dataset of runs from my Philadelphia Marathon training plan from both Strava and Garmin Connect. I then inspect the data to make sure I have all the correct data in the correct data types.
...
There were a lot more runs in this year's prep, as I was attempting to reach some higher weeky mileage, so there were a lot of double days (pretty much all double recovery runs). And, even though I missed some days completely due to some injuries, I still had almost 40 more runs in this time around. I believe that I was injured thanks to running my "easy" runs too hard, and to not having as long of a base building period prior to this marathon plan compared to last year (more on that later).

Looking above at the variables, I see that all the `as.POSIXct` fields did not load in as such, and `Month` was also not loaded as an ordered factor like I need it to be. As it is now, if I used `Month` in plots, it would be ordered alphabetically, not chronologically. So I need to convert `Month` back to an ordered factor with the correctly-specified ordering via the `levels` argument.
...
Now its time to actually do some analysis.
...
Just like [last year](https://stevenewns.netlify.com/2017/04/18/marathon-training-plan-data-analysis/), I again can see the actual marathon itself and my one 24 mile run, the longest **Long Run** of the plan, all the way to the right of the plot. So, this distribution is right/positively-skewed, with the majority of the runs being less than 12 miles. Also, as expected from to the higher mileage plan, the 2017 histogram has some higher bars, indicating a larger number of runs overall, with bars for the same distance being noticebaly higher for 2017 compared to 2016.

The majority of my runs this 2017 training block were around 4-6 miles, with the next highest frequency being from 10-12 miles. The fact that most of my runs were from 4-6 miles is not suprising when noting that the majority of my runs were recovery runs, which went from a minimum of 3 miles up to a maximum of 7 miles (for them most part, as sometimes I combined two recovery runs into one run, something I realized later may have been a mistake).
...
The above shows that for 2017, I actually had less workouts, long runs, and middle long runs (runs longer than a general aerobic run but shorter than a long run), but more recovery runs and general aerobic runs. This makes me think that I would've been taking it a bit easier overall in this build-up, with a higher number of "easier" runs, even though some of the lower numbers for the workouts were due to me missing these runs thanks to injury. For example, let's look at those runs that fall under `Misc`:
```{r Misc runs, echo = F}
philly %>%
  filter(RunCat == "Misc")
```

4 of these runs were the warm up and cool downs to tune-up races, but the other 2 were runs I kept short due to injury, one physical, one mental. I strained by quad twice in this build-up, and I took a couple of days off in each instance as I started to realize that I didn't have to hit *every* run on the schedule during the span of this plan, as noted by the empty spaces in the middle of September and October.
...
Let's look at the summary of both marathons' distance data:
...
cat("2016 Runs: ")
philly %>% filter(Marathon == "ph16") %>%
  select(Distance) %>%
  summarize(mean = mean(Distance), median = median(Distance), StdDev = sd(Distance),
            Min = min(Distance), Max = max(Distance), Count = n())
```

So, since I have a skewed distribution, I'd want to look at median to compare measures of center. I can see the median number of miles ran in a run for 2017 is 7 miles, which is lower than 10 miles for last year, which I can infer was due to more of the shorter recovery runs this time around, especially during the double-run days. And again, we see that the max distance here is, of course, the actual marathon (but something must've been up with my GPS watch or the course, as it says I ran about .15 miles moreso than the actual race distance). 
...
Somewhat against expectations, my mile total in November was actually shorter this time around, while the other months all did have higher totals. It makes sense in hindsight that September and October had lower totals than August, as August was the only month in which I did not take time off due to injury. But, I believe I also took some more days off in November this year because I was feeling quite fatigued and was worried I would go into the race too beat up, which I still think I did in the end (but it could've been worse, right?).
...
philly %>%
  ...
  ggplot(data = , aes(as.POSIXct(paste(month(Date),'/',day(Date), sep=""),format="%m/%d"), Avg.HR)) + 
    geom_line(aes(group=1, colour = Marathon)) +
    geom_smooth(method = "lm", se = T, aes(colour = Marathon)) + 
...
ggplot(data = philly, aes(Month, Avg.HR)) + 
  ...
  xlab("Month") + 
  ylab("Avg. Heart Rate") + 
  ggtitle("Average Heart Rate per Month") + facet_grid(~ Marathon)
```
